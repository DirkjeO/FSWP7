...
GDPdef <- MAGNET1_2 %>%
  ...
  mutate(variable = ifelse(unit == "mn USD", "GDPval", variable)) %>%
...
MAGNET3_raw[["XPRX"]] <- MAGNET1_2 %>%
  ...
  mutate(variable = ifelse(unit == "mn USD", "EXPOval", variable)) %>%
  select(-unit) %>%
  spread(variable, value) %>%
  left_join(., GDPdef) %>%
...
MAGNET3_raw[["NETT"]] <- MAGNET1_2 %>%
  filter(variable %in% c("EXPO", "IMPO") & unit == "mn USD") %>%
  spread(variable, value) %>%
...
# Nutrients
AV[["AV1"]] <- MAGNET1_2 %>%
  filter(variable %in% c("NQT", "POPT") & unit %in% c("CAL", "mn pers")) %>%
  group_by(scenario, FSregion, FSsector, year) %>%
...
GDPdef <- MAGNET1_2 %>%
  filter(variable %in% c("GDPT")) %>%
  mutate(variable = ifelse(unit == "bn USD 2007 MER", "GDPval", variable)) %>%
  select(-unit, -FSsector) %>%
  spread(variable, value)  
...
XPRP_p <- MAGNET1_2 %>%
  ...
  mutate(variable = ifelse(unit == "mn USD", "PRODval", variable)) %>%
  select(-unit) %>%
  spread(variable, value) %>%
  left_join(., GDPdef) %>%
...
XFPI_p <- MAGNET1_2 %>%
  ...
  mutate(variable = ifelse(unit == "mn USD", "PCONSval", variable)) %>%
  select(-unit) %>%
  spread(variable, value) %>%
  left_join(., GDPdef) %>%
...
AC[["XPRM"]] <-  MAGNET1_2 %>%
  ...
  mutate(variable = ifelse(unit == "mn USD", "VIPMval", variable)) %>%
  select(-unit) %>%
  spread(variable, value) %>%
  left_join(., GDPdef) %>%
...
ST[["IMDR"]] <- MAGNET1_2 %>%
 filter(variable %in% c("PROD", "EXPO", "IMPO") &  FSsector %in% c("CER") & unit == "mn USD") %>%
 group_by(scenario, FSregion, FSsector, year) %>%
...
ST[["SHRM"]] <- MAGNET1_2 %>% 
  filter((variable %in% c("EXPO") & FSsector %in% c("TOT") & unit == "mn USD") |
         (variable %in% c("IMPO") & FSsector %in% c("FOOD") & unit == "mn USD")) %>%
  group_by(scenario, FSregion, year) %>%
...
write.csv(MAGNET_tot, file.path(FSMIPPath, "MAGNET_qpc_ti4_st.csv"), row.names = F)
